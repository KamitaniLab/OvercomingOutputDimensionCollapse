...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Evaluate MSE between features and decoded features')
...
def get_configuration(args) -> Dict[str, Any]:
    ...
    return {
        ...
        "paths": {
            ...
            "decoded_feature_root": Path(f"{args.output_dir}/{args.test_dataset_name}/{args.feature_type}/{args.setting}"),
        }
    }
...
def setup_data_loader(config: Dict[str, Any], to_layer_name: dict) -> Tuple[FeaturesDataset, FeaturesDataset, list]:
    """Setup data loader"""
...
def calculate_mse(features, decoded_features, layer_names):
    ...
    concated_features = np.concatenate([features[layer_name].flatten() for layer_name in layer_names])
    concated_decoded_features = np.concatenate([decoded_features[layer_name].flatten() for layer_name in layer_names])
...
def process_stimulus(idx: int, features, decoded_features, stimulus_name, 
                    layer_names):
    ...
    missing_decoded_layers = [layer for layer in layer_names if layer not in decoded_features]
    if missing_decoded_layers:
        raise ValueError(f"Missing layers in decoded features: {missing_decoded_layers}")
    ...
    for idx in range(len(stimulus_names)):
        try:
            ...
            mse = process_stimulus(
                idx, features, decoded_features, stimulus_name,
                layer_names
            )
            mse_list.append(mse)
