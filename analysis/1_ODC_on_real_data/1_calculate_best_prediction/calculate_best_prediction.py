...
from bdpy.dataform import save_array, Features
...
def get_config(args):
    ...
    configs = {
        ...
        'results_dir_projection': Path(f"{args.output_dir}/{args.test_dataset_name}/best-prediction-feature/{args.setting}"),
...
def process_stimulus(idx: int, features, stimulus_name, layer_names, feature_selector, 
                    distortion_model, results_dir_projection, to_path_name):
...
if __name__ == "__main__":
    ...
    # Setup data loader
    data_loader, stimulus_names = setup_data_loader(configs['feature_root_path'], to_layer_name)
    
    # Load distortion model
    print("Loading distortion model...")
    if not configs['pseudo_inverse_dir'].exists():
        raise FileNotFoundError(f"Pseudo inverse directory not found: {configs['pseudo_inverse_dir']}")
    ...
    for idx, data in enumerate(data_loader):
        try:
            ...
            process_stimulus(
                idx, data, stimulus_name, layer_names, feature_selector,
                distortion_model, configs['results_dir_projection'], to_path_name
            )
