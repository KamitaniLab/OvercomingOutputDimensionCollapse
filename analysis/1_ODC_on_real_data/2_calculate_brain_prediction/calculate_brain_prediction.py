...
def parse_arguments():
    ...
    parser.add_argument('--output_dir', type=str, default='./assets/1_ODC_on_real_data/features',
                       help='Output directory for results')
    parser.add_argument('--test_dataset_name', type=str, default='ImageNetTest',
                       help='Test dataset name')
    parser.add_argument('--proc_dir', type=str, default='./assets/1_ODC_on_real_data/calculate_best_prediction',
                       help='Directory for processing')
    parser.add_argument('--ridge_alpha', type=float, default=1000.0,
    ...
    parser.add_argument('--roi', type=str, default='VC',
                       help='ROI to use for brain data (default: VC)')
...
def get_config(args):
    ...
    configs = {
        'results_dir_projection': Path(f"{args.output_dir}/{args.test_dataset_name}/decoded-feature-ridge-alpha{args.ridge_alpha}/{args.setting}"),
...
def get_feature_selector(feature_map: dict):
    ...
    for layer_name, feature_shape in feature_map.items():
        feature_map_slice_idx[layer_name] = slice(slice_idx, slice_idx + np.prod(feature_shape))
        slice_idx += np.prod(feature_shape)
    
    def feature_selector(feature, layer_name: str):
        selected_feature = feature[feature_map_slice_idx[layer_name]].reshape(feature_map[layer_name])
...
def load_decoded_data(proc_dir: Path, setting: str, test_dataset_name: str, ridge_alpha: float):
    """Load decoded data"""
    print("Loading decoded data...")
    
    # Load decoded I dataset
    input_path = proc_dir / setting / test_dataset_name / f"ridge-alpha{ridge_alpha}/I_pred.pkl"
    with input_path.open("rb") as f:
        decoded_I_dataset = pickle.load(f)
    print(f"decoded_I_dataset.shape: {decoded_I_dataset.shape}")

    # Load labels
    input_path = proc_dir / setting / test_dataset_name / f"ridge-alpha{ridge_alpha}/labels.pkl"
    with input_path.open("rb") as f:
...
def process_stimulus(idx: int, decoded_I, label, layer_names, feature_selector, 
                    projector, results_dir_projection, to_path_name, subject: str, roi: str):
    ...
    for layer_name in layer_names:
        ...
        save_dir = os.path.join(results_dir_projection, to_path_name[layer_name], subject, roi)
...
if __name__ == "__main__":
    ...
    # Load decoded data
    decoded_I_dataset, labels = load_decoded_data(Path(args.proc_dir), args.setting, args.test_dataset_name, args.ridge_alpha)
    ...
    if not configs['concatenated_feature_dir'].exists():
        raise FileNotFoundError(f"Concatenated feature directory not found: {configs['concatenated_feature_dir']}")
    ...
    for idx, (decoded_I, label) in enumerate(data_loader):
        try:
            process_stimulus(
                idx, decoded_I, label, layer_names, feature_selector,
                projector, configs['results_dir_projection'], to_path_name, configs['subject'], configs['roi']
            )
