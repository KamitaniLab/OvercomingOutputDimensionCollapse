...
def parse_arguments():
    ...
    parser.add_argument('--brain_train_dir', type=str, 
                       default='./data/fmri/',
                       help='Directory to training brain data')
    parser.add_argument('--brain_test_dir', type=str, 
                       default='./data/fmri/',
                       help='Directory to test brain data')
    parser.add_argument('--feature_dir', type=str, default='./data/features/ImageNetTraining/caffe/VGG19/',
...
def get_config(args):
    ...
    configs = {
        ...
        'data_brain_train_path': Path(args.brain_train_dir) / f"{args.subject}_ImageNetTraining_volume_native.h5",
        'data_brain_test_path': Path(args.brain_test_dir) / f"{args.subject}_{args.test_dataset_name}_volume_native.h5",
        'roi': f"ROI_{args.roi}",
        'feature_label_dir': Path(args.feature_dir),
        'assets_root_path': Path(f"{args.output_dir}/{args.setting}/{args.test_dataset_name}/ridge-alpha{args.alpha}"),
    }
...
def load_brain_data(data_brain_train_path: Path, data_brain_test_path: Path, roi: str):
    ...
    x_test = np.vstack([np.mean(x_test[(np.array(x_test_labels) == xl).flatten(),:], axis=0) for xl in x_test_labels_unique])
...
def prepare_training_data(x_train: np.ndarray, x_train_labels: list, idxs: list, y_labels: list):
...
def train_and_predict(x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, alpha: float):
...
def save_results(assets_root_path: Path, x_train_mean: np.ndarray, x_train_std: np.ndarray, 
                model: Ridge, y_pred: np.ndarray, x_test_labels_unique: np.ndarray):
...
if __name__ == "__main__":
    ...
    x_train, x_train_labels, x_test, x_test_labels_unique = load_brain_data(
        configs['data_brain_train_path'], configs['data_brain_test_path'], configs['roi']
    )
    ...
    x_train, y_train = prepare_training_data(x_train, x_train_labels, idxs, selected_labels)
    ...
    # Save results
    save_results(configs['assets_root_path'], x_train_mean, x_train_std, model, y_pred, x_test_labels_unique)
