...
DATASETS = {
    "natural": "ImageNetTest",
    "artificial": "ArtificialShapes"
}
FEATURE_TYPES = {
...
def load_data():
    ...
    projected_feature_files = [pd.read_pickle(file_name) for file_name in projected_feature_file_names]
    decoded_feature_files = [pd.read_pickle(file_name) for file_name in decoded_feature_file_names]
    projected_feature_artificial_files = [pd.read_pickle(file_name) for file_name in projected_feature_artificial_file_names]
    decoded_feature_artificial_files = [pd.read_pickle(file_name) for file_name in decoded_feature_artificial_file_names]
    
    return (projected_feature_files, decoded_feature_files, 
            projected_feature_artificial_files, decoded_feature_artificial_files)
# %%
def plot_mse_line_chart(projected_feature_files, projected_feature_artificial_files, 
                        sample_names=None, figsize=(12, 6), xlim=(0, 90)):
    """
    ...
    projected_feature_mses = [projected_feature_file["mse"] for projected_feature_file in projected_feature_files]
    projected_feature_mses_artificial = [projected_feature_artificial_file["mse"] for projected_feature_artificial_file in projected_feature_artificial_files]
    projected_feature_mses_concatenated = [np.concatenate([projected_feature_mses[i], projected_feature_mses_artificial[i]]) for i in range(len(projected_feature_mses))]

    # Sort by 150 samples (smallest sample size) MSE values
    sorted_indices = np.argsort(projected_feature_mses_concatenated[-1])
    projected_feature_mses_concatenated_sorted = [mse_values[sorted_indices] for mse_values in projected_feature_mses_concatenated]

    # Create line plot
    plt.figure(figsize=figsize, dpi=300)
    indices = np.arange(len(projected_feature_mses_concatenated_sorted[0]))

    for i, mse_values in reversed(list(enumerate(projected_feature_mses_concatenated_sorted))):
    ...
    plt.show()
# %%
def plot_error_percentage_comparison(projected_feature_file, decoded_feature_file, 
                                   projected_feature_artificial_file, decoded_feature_artificial_file,
                                   figsize=(6, 12), ylim=(0, 100)):
    """
    ...
    projected_error_percentage_artificial = (projected_error_artificial / decoded_error_artificial) * 100

    # Create bar plot
    plt.figure(figsize=figsize, dpi=300)
    plt.bar("Natural images", projected_error_percentage.mean(), 
            yerr=projected_error_percentage.std(), width=0.8, capsize=4, color='#5A79A5')
    plt.bar("Artificial images", projected_error_percentage_artificial.mean(), 
            yerr=projected_error_percentage_artificial.std(), width=0.8, capsize=4, color='#C69756')
...
# %%
def plot_mse_scatter(projected_feature_file, decoded_feature_file, 
                    projected_feature_artificial_file, decoded_feature_artificial_file,
                    figsize=(10, 10), natural_color='#5A79A5', artificial_color='#C69756'):
    """
    ...
    # Plot natural images
    plt.scatter(projected_feature_file["mse"], decoded_feature_file["mse"], 
                s=25, color=natural_color, label="Natural images")
    
    # Plot artificial images
    plt.scatter(projected_feature_artificial_file["mse"], decoded_feature_artificial_file["mse"], 
                s=25, color=artificial_color, marker='s', label="Artificial images")
    
    # Add diagonal line
    max_val = max(projected_feature_file["mse"].max(), decoded_feature_file["mse"].max(), 
                  projected_feature_artificial_file["mse"].max(), decoded_feature_artificial_file["mse"].max()) * 1.05
    plt.xlim(0, max_val)
...
projected_feature_files, decoded_feature_files, projected_feature_artificial_files, decoded_feature_artificial_files = load_data()
...
plot_error_percentage_comparison(projected_feature_files[0], decoded_feature_files[0], 
                                projected_feature_artificial_files[0], decoded_feature_artificial_files[0])

# %%
# Plot scatter plots for all sample sizes
for i, (projected_feature_file, decoded_feature_file, projected_feature_artificial_file, decoded_feature_artificial_file) in enumerate(zip(projected_feature_files, decoded_feature_files, projected_feature_artificial_files, decoded_feature_artificial_files)):
    sample_names = ["1200 samples", "600 samples", "300 samples", "150 samples"]
    print(f"Plotting {sample_names[i]}")
    plot_mse_scatter(projected_feature_file, decoded_feature_file, 
                    projected_feature_artificial_file, decoded_feature_artificial_file)
