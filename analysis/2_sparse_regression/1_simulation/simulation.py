from overcoming_output_dimension_collapse.sparse_regression.fastl2lir_pro import FastL2LiR
...
def create_argument_parser():
    ...
    parser.add_argument('--d_in', type=int, default=1000, help='Number of input features')
    parser.add_argument('--d_out', type=int, default=1000, help='Number of output targets')
    parser.add_argument('--n_test', type=int, default=1000, help='Number of test samples')
    parser.add_argument('--nonzero_ratio', type=float, default=0.01, help='ratio of nonzero elements in W')
    parser.add_argument('--noise_std', type=float, default=0.1, help='noise standard deviation')
    parser.add_argument('--n_trials', type=int, default=100, help='Number of trials')
    parser.add_argument('--data_type', type=str, default='baseline', 
                       choices=['baseline', 'gaussian-weight', 'correlated-signal', 'input-noise'],
                       help='Type of data generation')
    parser.add_argument('--rho', type=float, default=0.5, help='Correlation parameter for correlated-signal')
    parser.add_argument('--output_dir', type=str, default='assets/2_sparse_regression/simulation',
...
def generate_data(data_type, d_in, d_out, n_train, n_test, nonzero_ratio, noise_std, rho=0.5):
    ...
    if data_type == 'baseline':
        ...
        W = M
        cov_matrix = np.array([[rho ** abs(i - j) for j in range(d_in)] for i in range(d_in)])
        X_train = np.random.multivariate_normal(np.zeros(d_in), cov_matrix, size=n_train)
...
def run_single_trial(trial_idx, data_type, d_in, d_out, n_train_max, n_test, 
                    nonzero_ratio, noise_std, rho, n_trains, settings, solver):
    ...
    for n_train in n_trains:
        ...
        Y_test_std = scalerY.transform(Y_test)  # (n_test, d_out)

        # Fit and predict models for different settings
        for setting in settings:
            if setting == 'select-optimal':
                p_sel, risk, tau2 = solver.solve_optimal_p_select_and_risk(nonzero_ratio, noise_std, n_train / d_in)
                Y_pred_std, n_feat = fit_predict_fastl2lir(
                    X_train_sub_std, Y_train_sub_std, X_test_std, 
                    n_feat=int(p_sel * d_in), alpha=d_in / tau2
                )
            elif setting == 'without-select-optimal':
                risk, tau2 = solver.solve_risk_and_tau2_for_p_select(nonzero_ratio, noise_std, n_train / d_in, 1.0)
...
def save_results(mse_dict, n_feat_dict, risk_dict, d_in, d_out, n_test, 
                nonzero_ratio, noise_std, n_trials, data_type, rho, output_dir):
...
if __name__ == "__main__":
    ...
    for trial in tqdm(range(n_trials), disable=False):
        trial_results = run_single_trial(
            trial, data_type, d_in, d_out, n_train_max, n_test,
            nonzero_ratio, noise_std, rho, n_trains, settings, solver
        )
    ...
    # Save results
    save_results(mse_dict, n_feat_dict, risk_dict, d_in, d_out, n_test,
                nonzero_ratio, noise_std, n_trials, data_type, rho, output_dir)