...
# Extract a portion of Reds to create a new colormap
reds_cmap = plt.cm.Reds
colors = reds_cmap(np.linspace(0.0, 0.7, 256))  # Use range from 0.0 to 0.7
...
def plot_risk_landscape(
    ...
    for i, p_sel in enumerate(p_sel_values):
        for j, pi in enumerate(pi_values):
            risk_values[i, j] = solver.risk_from_p_sel_and_pi(
                p_sel, pi, sigma, data_scale
            )
...
def plot_pi_curves(
    ...
    for idx, a in enumerate(a_list):
        pi_line = [
            solver.solve_pi_given_a_p_select_and_data_scale(a, p_sel, data_scale, sigma)
...
def compute_optimal_results(sigma, a_vals, data_scale_vals):
    ...
    naive_results = [
        (
            ...
            data_scale,
            1,
            *solver.solve_risk_and_tau2_for_p_select(1, sigma, data_scale, 1),
        )
    ...
    df_results = pd.DataFrame(
        results, columns=["a", "data_scale", "p_select", "risk", "tau2"]
    )
    return df_results


# %%
def plot_risk_vs_data_scale(
    sigma,
    a_vals,
    data_scale_vals=None,
    figsize=(10, 7),
    line_width=3,
    ylim=(0, 1.1),
    show_grid=True,
    ...
    # Plot curves for each a value
    for a, color in zip(a_vals, colors, strict=False):
